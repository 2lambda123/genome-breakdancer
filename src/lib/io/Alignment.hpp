...
inline bool Alignment::leftmost() const { return _pos < _mpos; }

inline void Alignment::set_bdflag(ReadFlag const &new_flag) {
  _bdflag = new_flag;
}

inline ReadFlag Alignment::bdflag() const { return _bdflag; }

inline uint8_t Alignment::bdqual() const { return _bdqual; }

inline int32_t Alignment::tid() const { return _tid; }

inline int32_t Alignment::pos() const { return _pos; }

inline int32_t Alignment::query_length() const { return _query_length; }

inline strand_e Alignment::ori() const {
  return sam_flag() & BAM_FREVERSE ? REV : FWD;
}

inline int32_t Alignment::abs_isize() const { return _abs_isize; }

inline std::string const &Alignment::query_name() const { return _query_name; }

inline void Alignment::set_lib_index(std::size_t const &index) {
  _lib_index = index;
}

inline std::size_t Alignment::lib_index() const {
  // assert(_lib_index != ~0ull);
  return _lib_index;
}

inline bool Alignment::has_sequence() const {
  return !_bam_data.empty() && query_length() > 0;
}

inline uint16_t Alignment::sam_flag() const { return _sam_flag; }

inline bool Alignment::proper_pair() const {
  static int const mask =
      BAM_FPROPER_PAIR | BAM_FUNMAP | BAM_FMUNMAP | BAM_FPAIRED | BAM_FDUP;
  static int const want = BAM_FPROPER_PAIR | BAM_FPAIRED;
  return (sam_flag() & mask) == want;
}

inline bool Alignment::either_unmapped() const {
  return sam_flag() & (BAM_FUNMAP | BAM_FMUNMAP);
}

inline bool Alignment::interchrom_pair() const { return _tid != _mtid; }
