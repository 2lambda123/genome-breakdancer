...
template <typename Filter>
inline RegionLimitedBamReader<Filter>::RegionLimitedBamReader(
    std::string const &path, char const *region)
    : BamReader<Filter>(path), _region(region), _path(path),
      _index(bam_index_load(path.c_str())), _iter(0) {
  using boost::format;
  if (!_index)
    throw std::runtime_error(
        str(format("Failed to load bam index for %1%") % path));

  set_region(region);
}

template <typename Filter>
inline void RegionLimitedBamReader<Filter>::set_region(char const *region) {
  using boost::format;

  _region = region;

  if (bam_parse_region(BamReader<Filter>::_in->header, region, &_tid, &_beg,
                       &_end) < 0) {
    throw std::runtime_error(
        str(format("Failed to parse bam region '%1%' for file %2%. ") % region %
            this->path()));
  }
...
template <typename Filter>
inline int RegionLimitedBamReader<Filter>::next(bam1_t *entry) {
  while (int rv =
             bam_iter_read(BamReader<Filter>::_in->x.bam, _iter, entry) > 0) {
    if (Filter()(entry))
      return rv;
  }
  return 0;
