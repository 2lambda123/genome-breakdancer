...
BamConfig::BamConfig() : _max_read_window_size(DEFAULT_MAX_READ_WINDOW_SIZE) {}
...
BamConfig::BamConfig(std::istream &in, int cutoff_sd)
  ...
  while (getline(in, line)) {
    ...
    // FIXME: why are we reading this as float from the config and storing as
    // int?
    ...
    pair<TmpIter, bool> inserted =
        temp_lib_config.insert(make_pair(lib, lib_config));
    if (!inserted.second && inserted.first->second != lib_config) {
      std::cerr << "WARNING: at line " << line_num << ", library " << lib
                << " overwritten!\n";
    ...
    int tmp =
        mean -
        readlen *
            2; // this determines the mean of the max of the SV flanking region
  ...
  for (IterType iter = _bam_library.begin(); iter != _bam_library.end();
       ++iter) {
  ...
  for (size_t i = 0; i < _library_config.size(); ++i) {
    ...
    vector<string>::const_iterator bam_iter =
        find(bam_files().begin(), bam_files().end(), lib.bam_file);

    if (bam_iter == bam_files().end())
      throw runtime_error(str(
          format("Bam file '%1%' referenced by library '%2%' but not found in "
                 "bam list!") %
          lib.bam_file % lib.name));
...
int BamConfig::max_read_window_size() const { return _max_read_window_size; }

size_t BamConfig::num_libs() const { return _library_config.size(); }

size_t BamConfig::num_bams() const { return _bam_files.size(); }
