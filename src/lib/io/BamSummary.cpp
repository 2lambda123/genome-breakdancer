...
BamSummary::BamSummary() : _covered_ref_len(0) {}
...
BamSummary::BamSummary(Options const &opts, BamConfig const &bam_config,
    ...
    : _covered_ref_len(0), _library_flag_distributions(bam_config.num_libs()),
...
LibraryFlagDistribution const &
BamSummary::library_flag_distribution(size_t libIdx) const {
...
void BamSummary::_analyze_bam(
    Options const &opts, BamConfig const &bam_config, BamReaderBase &reader,
    IAlignmentClassifier const &alignment_classifier) {
  int last_pos = 0;
  int last_tid = -1;
  ...
  AlignmentSource src(reader, alignment_classifier, bam_config,
  ...
  while (Alignment::Ptr alnptr = src.next()) {
    ...
    LibraryConfig const &lib_config =
        bam_config.library_config(aln.lib_index());
    ...
    LibraryFlagDistribution &lib_flag_dist =
        _library_flag_distributions[lib_config.index];
    if (aln.proper_pair()) {
...
void BamSummary::_analyze_bams(
    Options const &opts, BamConfig const &bam_config,
    IAlignmentClassifier const &alignment_classifier) {
  std::vector<std::string> bam_files = bam_config.bam_files();
  for (std::vector<std::string>::const_iterator iter = bam_files.begin();
       iter != bam_files.end(); ++iter) {
