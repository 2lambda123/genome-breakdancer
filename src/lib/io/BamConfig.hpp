...
public:
  ...
  static const std::string
      EMPTY_STRING; // Want to occasionally return an empty string
...
inline LibraryConfig const &BamConfig::library_config(size_t idx) const {
  if (idx >= _library_config.size())
    throw std::out_of_range("library index out of range");
  return _library_config[idx];
}

inline LibraryConfig const &
BamConfig::library_config(std::string const &lib) const {
  return _library_config[_lib_names_to_indices.at(lib)];
}

inline std::string const &
BamConfig::readgroup_library(std::string const &rg) const {
  ConfigMap<std::string, std::string>::type::const_iterator lib =
      _readgroup_library.find(rg);
...
void BamConfig::serialize(Archive &arch, const unsigned int version) {
  ...
  arch &bs::make_nvp("bamLibrary", _bam_library) &
