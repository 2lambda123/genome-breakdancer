...
ConfigBuilder::ConfigBuilder(std::ostream &out, std::ostream *dist_out,
                             std::vector<std::string> const &bam_paths,
                             std::size_t min_mapq, std::size_t min_observations,
                             double n_devs, double n_mads,
                             std::size_t no_progress_limit, std::size_t skip,
                             std::vector<std::string> regions,
                             bool verbose // = false
                             )
    : out_(out), dist_out_(dist_out), bam_paths_(bam_paths),
      min_mapq_(min_mapq), min_observations_(min_observations), n_devs_(n_devs),
      n_mads_(n_mads), no_progress_limit_(no_progress_limit), skip_(skip),
      regions_(regions), verbose_(verbose) {
  // write distribution table header
  if (dist_out_)
    *dist_out_ << "bam_path\tread_group\tlibrary\tinsert_size\tcount\n";
}

std::unique_ptr<BamReaderBase>
ConfigBuilder::open_bam(std::string const &path) {
  if (!regions_.empty()) {
    if (skip_ != 0) {
      std::cerr << "Warning: ignoring --skip option since regions are given\n";
    }
    return make_unique_<
        MultiRegionLimitedBamReader<AlignmentFilter::IsPrimary>>(path,
                                                                 regions_);
  } else {
...
void ConfigBuilder::process_bam(BamReaderBase &reader) {
    ...
    if (found == rg_remaining.end()) {
      ...
      if (++no_progress_counter >= no_progress_limit_) {
        ...
        ss << "Note: this limit can be adjusted by the " << "--"
           << NO_PROGRESS_COUNT_CMDLINE_PARAM_NAME << " parameter.";
  ...
  for (auto i = dists.begin(); i != dists.end(); ++i) {
    ...
    std::cerr << "Read group " << i->first << ": ignoring insert sizes above "
              << limit << "\n";
    ...
    out_ << "readgroup:" << i->first << "\tplatform:illumina"
         << "\tmap:" << reader.path() << "\treadlen:" << readlen
         << "\tlib:" << rg_lib[i->first] << "\tnum:" << n
         << "\tlower:" << std::max(0.0, mean - n_devs_ * sd_lo)
         << "\tupper:" << mean + n_devs_ * sd_hi << "\tmean:" << mean
         << "\tstd:" << sd << "\n";
...
void ConfigBuilder::write_distribution(
    std::string const &bam_file, std::string const &read_group,
    std::string const &library, CountsDistribution<std::size_t> const &dist) {
  if (!dist_out_)
    return;
