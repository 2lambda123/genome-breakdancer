...
class ConfigBuilder {
public:
  ConfigBuilder(std::ostream &out, std::ostream *dist_out,
                std::vector<std::string> const &bam_paths, std::size_t min_mapq,
                std::size_t min_observations, double n_devs, double n_mads,
                std::size_t no_progress_limit, std::size_t skip,
                std::vector<std::string> regions, bool verbose = false);
...
protected:
  ...
  void write_distribution(std::string const &bam_file,
