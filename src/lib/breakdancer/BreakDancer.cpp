...
real_type ComputeProbScore(int total_region_size,
                           map<size_t, int> &rlibrary_readcount, ReadFlag type,
                           int fisher, LibraryInfo const &lib_info) {
  real_type lambda;
  real_type logpvalue = 0.0;
  real_type err = 0.0;
  for (map<size_t, int>::const_iterator ii_rlibrary_readcount =
           rlibrary_readcount.begin();
       ii_rlibrary_readcount != rlibrary_readcount.end();
       ii_rlibrary_readcount++) {
    ...
    LibraryFlagDistribution const &lib_flags =
        lib_info._summary.library_flag_distribution(lib_config.index);

    uint32_t read_count_for_flag = lib_flags.read_counts_by_flag[type];
    lambda = real_type(total_region_size) *
             (real_type(read_count_for_flag) /
              real_type(lib_info._summary.covered_reference_length()));
...
BreakDancer::BreakDancer(IAlignmentClassifier const &read_classifier,
                         Options const &opts, LibraryInfo const &lib_info,
                         ReadRegionData &read_regions,
                         BamReaderBase &merged_reader, int max_read_window_size)
    : _read_classifier(read_classifier), _opts(opts), _lib_info(lib_info),
      _rdata(read_regions), _merged_reader(merged_reader),
      _max_read_window_size(max_read_window_size)

      ,
      _collecting_normal_reads(false), _nnormal_reads(0),
      _ntotal_nucleotides(0), _max_readlen(0), _buffer_size(0)

      ,
      _region_start_tid(-1), _region_start_pos(-1), _region_end_tid(-1),
      _region_end_pos(-1) {
  ...
  if (!_opts.dump_BED.empty()) {
    ...
    _bed_writer.reset(
        new BedWriter(*_bed_stream, _lib_info, _merged_reader.header()));
  }
}

void BreakDancer::run() {
  AlignmentSource src(_merged_reader, _read_classifier, _lib_info._cfg,
...
void BreakDancer::push_read(Alignment::Ptr const &alnptr) {
  ...
  LibraryConfig const &lib_config =
      _lib_info._cfg.library_config(aln.lib_index());
  ...
  // Store readdepth in nread_ROI by bam name (no per library calc) or by
  // library I believe this only counts normally mapped reads
  if (aln.proper_pair()) {
    string const &key =
        _opts.CN_lib == 1 ? lib_config.name : lib_config.bam_file;
    _rdata.incr_normal_read_count(key);
  }

  // for long insert
  // Mate pair libraries have different expected orientations so adjust
  // Also, aligner COULD have marked (if it was maq) that reads had abnormally
  // large or small insert sizes Remark based on BD options
  // FIXME: make mate-pair alignment classifier class
  if (_opts.Illumina_long_insert) {
    if (aln.abs_isize() > lib_config.uppercutoff &&
        aln.bdflag() == ReadFlag::NORMAL_RF) {
      aln.set_bdflag(ReadFlag::ARP_RF);
    }
    if (aln.abs_isize() < lib_config.uppercutoff &&
        aln.bdflag() == ReadFlag::ARP_RF) {
      aln.set_bdflag(ReadFlag::NORMAL_RF);
    }
    if (aln.abs_isize() < lib_config.lowercutoff &&
        aln.bdflag() == ReadFlag::NORMAL_RF) {
  ...
  // count reads mapped by SW, FR and RF reads, but only if normal_switch is
  // true normal_switch is set to 1 as soon as reads are accumulated for dumping
  // to fastq??? Not sure on this. Happens later in this function I suspect this
  // is to include those reads in the fastq dump for assembly!
  if (aln.bdflag() == ReadFlag::NORMAL_FR ||
      aln.bdflag() == ReadFlag::NORMAL_RF) {
  ...
  // This appears to test that you've exited a window after your first abnormal
  // read by either reading off the chromosome or exiting the the window
  //  d appears to be 1e8 at max (seems big), 50 at minimum or the smallest mean
  //  - readlen*2 for a given library
  bool do_break = aln.tid() != _region_end_tid ||
                  aln.pos() - _region_end_pos > _max_read_window_size;
  ...
  reads_in_current_region.push_back(
      alnptr); // store each read in the region_sequence buffer

  // If we just added the first read, flip the flag that lets us collect all
  // reads
...
void BreakDancer::process_breakpoint() {
  float seq_coverage =
      _ntotal_nucleotides /
      float(_region_end_pos - _region_start_pos + 1 + _max_readlen);
  if (_region_end_pos - _region_start_pos > _opts.min_len &&
      seq_coverage < _opts.seq_coverage_lim) // skip short/unreliable flanking
                                             // supporting regions
  {
    // register reliable region and supporting reads across gaps
    // int region_idx = _rdata.add_region(new BasicRegion(_region_start_tid,
    // _region_start_pos, _region_end_pos, _nnormal_reads));
    // add_current_read_counts_to_last_region();
    _rdata.add_region(_region_start_tid, _region_start_pos, _region_end_pos,
                      _nnormal_reads, reads_in_current_region);
...
void BreakDancer::build_connection() {
  ...
  for (vector<int>::const_iterator i = active_nodes.begin();
...
void BreakDancer::process_sv(std::vector<int> const &snodes) {
  ...
  using boost::bind;
  boost::function<bool(Alignment::Ptr const &)> is_supportive =
      bind(std::equal_to<size_t>(), 0,
           bind(&SvBuilder::ObservedReads::count, &svb.observed_reads,
                bind(&Alignment::query_name, _1)));
  ...
  if (snodes.size() == 2) {
    _rdata.accumulate_reads_between_regions(read_count_accumulator, snodes[0],
                                            snodes[1]);
  }
  svb.compute_copy_number(read_count_accumulator, _read_density);

  if (svb.flag != ReadFlag::ARP_RF && svb.flag != ReadFlag::ARP_RR &&
      svb.pos[0] + _max_readlen - 5 < svb.pos[1])
    svb.pos[0] +=
        _max_readlen - 5; // apply extra padding to the start coordinates

  string sptype_tmp;
  float diff = 0;
  if (_opts.CN_lib == 1) {
    for (map<size_t, int>::const_iterator ii_type_lib_rc =
             svb.type_library_readcount[svb.flag].begin();
      ...
      sptype_tmp += lib_config.name + "|" + lexical_cast<string>(read_count) +
                    "," + copy_number_str;

      diff += float(svb.type_library_meanspan[svb.flag][index]) -
              float(svb.type_library_readcount[svb.flag][index]) *
                  lib_config.mean_insertsize;
    }
    ...
    for (map<string, int>::const_iterator ii_type_bam_rc =
             type_bam_readcount.begin();
         ii_type_bam_rc != type_bam_readcount.end(); ii_type_bam_rc++) {
  ...
  real_type LogPvalue =
      ComputeProbScore(total_region_size, svb.type_library_readcount[svb.flag],
                       svb.flag, _opts.fisher, _lib_info);
  ...
  if (PhredQ > _opts.score_threshold) {
    ...
    cout << bam_header->target_name[svb.chr[0]] << "\t" << svb.pos[0] << "\t"
         << svb.fwd_read_count[0] << "+" << svb.rev_read_count[0] << "-" << "\t"
         << bam_header->target_name[svb.chr[1]] << "\t" << svb.pos[1] << "\t"
         << svb.fwd_read_count[1] << "+" << svb.rev_read_count[1] << "-" << "\t"
         << svb.sv_type() << "\t" << svb.diffspan << "\t" << PhredQ << "\t"
         << svb.flag_counts[svb.flag] << "\t" << sptype;
    ...
    if (_opts.CN_lib == 0 && svb.flag != ReadFlag::ARP_CTX) {
      ...
      for (vector<string>::const_iterator iter = bams.begin();
           iter != bams.end(); ++iter) {
...
void BreakDancer::dump_fastq(ReadFlag const &flag,
  ...
  for (auto i = support_reads.begin(); i != support_reads.end(); ++i) {
    ...
    // Paradoxically, the first read seen is put in file 2 and the second in
    // file 1
    bool is_read1 = pairing.count(y.query_name()) != 0;
    std::string const &libname =
        _lib_info._cfg.library_config(y.lib_index()).name;
